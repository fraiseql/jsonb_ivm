...
**Complete CRUD workflow examples for incremental view maintenance using jsonb_ivm v0.3.0**
...
This guide shows how to use `jsonb_ivm` functions in a CQRS architecture with pg_tview-style projection tables. Each example demonstrates the **before** (traditional SQL) and **after** (jsonb_ivm) approaches.

### What's Different in v0.3.0?

- ✅ **Complete CRUD**: INSERT and DELETE now available (were missing in v0.2.0)
- ✅ **Smart Patch API**: Unified `jsonb_smart_patch_*()` functions simplify dispatch logic
- ✅ **Deep Merge**: `jsonb_deep_merge()` for recursive nested updates
- ✅ **Helper Functions**: `jsonb_extract_id()` and `jsonb_array_contains_id()` for cleaner code
...
- `test/smoke_test_v0.3.0.sql` - Quick validation tests
